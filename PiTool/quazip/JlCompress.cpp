...
bool JlCompress::compressDir(QString fileCompressed, QString dir,QString subDir, bool recursive,QDir::Filters filters)
{
    // Creo lo zip
    QuaZip zip(fileCompressed);
    QDir().mkpath(QFileInfo(fileCompressed).absolutePath());
    ...
    // Aggiungo i file e le sotto cartelle
    if (!compressSubDir(&zip,subDir,dir,recursive, filters)) {
        QFile::remove(fileCompressed);
        return false;
    }
